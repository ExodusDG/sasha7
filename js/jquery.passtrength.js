    ...
    Plugin.prototype = {
        ...
        addStatus: function(percentage) {
            ...
            if (percentage >= 25) {
                ...
                $('#pswd-lvl-1').attr('style', 'background: red')
                $('.password-level-status').not('#pswd-lvl-1').attr('style', 'background: #aaa')
            }
            if (percentage >= 50) {
                meter.attr("class", "passtrengthMeter");
                //  status = "medium";
                text = this.options.textMedium;
                $('#pswd-lvl-2').attr('style', 'background: orange')
            }
            if (percentage >= 75) {
                meter.attr("class", "passtrengthMeter");
                //  status = "strong";
                text = this.options.textStrong;
                $('#pswd-lvl-3').attr('style', 'background: yellow')
            }
            if (percentage >= 100) {
                meter.attr("class", "passtrengthMeter");
                //  status = "very-strong";
                text = this.options.textVeryStrong;
                $('#pswd-lvl-4').attr('style', 'background: green')
            }
